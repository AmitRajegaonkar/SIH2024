...
def id_face(input_buffer:multiprocessing.Queue,exited:ValueProxy,criminals_db:DictProxy): # type: ignore
    ...
    while not exited.get():
        ...
        user_types = []
        for encodings,user_type in temp:
                ...
                try:
                    if flag:
                        ...
                        with open("frame.jpg",'rb') as f:
                            ...
                            print(response['id'])
                            to_add = s.post(f"{direct_us_url}/items/missing_alert",headers=headers,json={"frame":response['id'],"location":location_camera,"missing_detected":name}).json()
                            print(to_add)
    ...
    while not exited.get():
        ...
        try:
            ...
            for i in range(len(criminals)):
                to_share[criminals[i]] = [encodings[i],'missing']
