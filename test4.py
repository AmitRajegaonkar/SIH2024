...
def image_capture(input_buffer:multiprocessing.Queue,camera:int,exited:ValueProxy,display_buffer:multiprocessing.Queue) -> None:
    ...
    while not exited.get():
        ...
        time.sleep(1) # For laptop webcam
                ...
                try:
                    if flag:
                        ...
                        with open("frame.jpg",'rb') as f:
                            response = s.post("http://172.27.229.77:8055/files",headers=headers,files={"file":("image.jpg", f, "image/jpeg")},data={"folder":"e23d691c-b205-4b75-9bd8-fb7693827857"}).json()['data']
                            print(response['id'])
                            to_add = s.post("http://172.27.229.77:8055/items/alert_db",headers=headers,json={"frame":response['id'],"camera_location":str(camera),"aadhaar_found":name}).json()
                            print(to_add)
...
def get_criminal_encodings(exited:ValueProxy,criminals_db:DictProxy):
    ...
    while not exited.get():
        ...
        try:
            s = requests.Session()
            criminals_aadhaar_full:list = s.get("http://172.27.229.77:8055/items/criminal_db",headers=headers).json()['data']
            ...
            for criminal in criminals:
                encodings_full:list = s.get(f'http://172.27.229.77:8055/items/aadhaar_db/{criminal}',headers=headers).json()['data']['face_embeddings']['data']
