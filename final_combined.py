...
def id_face(input_buffer:multiprocessing.Queue,exited:ValueProxy,criminals_db:DictProxy): # type: ignore
    ...
    while not exited.get():
        ...
        location_camera = "Bhopal Railway Station"
            ...
            if matches[best_match_index]:
                ...
                try:
                    if flag:
                        ...
                        headers = {"Authorization": "Bearer 7k8V9rO8RtGRTYAeaN-NB2jVQ-DP9fzL"}
                        sus_aadhaar_details:dict = s.get(f"{direct_us_url}/items/aadhaar_db/{sus_aadhaar}",headers=headers).json().get('data',{"name":"UNK","address":"UNK","DOB":"UNK","phone_no":"UNK","face_embedding":[],"user_email":"UNK"})
                        ...
                        with open("frame.jpg",'rb') as f:
                            ...
                            print(response['id'])
                            to_add = s.post(f"{direct_us_url}/items/criminal_alert",headers=headers,json={"frame":response['id'],"location":location_camera,"criminal_detected":name,"suspect_aadhaar":sus_aadhaar,"aadhaar_notes":notes,'cctv_location':"POINT (77.3133 23.1453)"}).json()
                            
                            print(name,location_camera,"POINT (77.3133 23.1453)",time.ctime(),"CRIMINAL")
                ...
                try:
                    if flag:
                        ...
                        with open("frame.jpg",'rb') as f:
                            response = s.post(f"{direct_us_url}/files",headers=headers,files={"file":("image.jpg", f, "image/jpeg")},data={"folder":"9ec2c9f6-3e72-432b-8173-5893fb3a63d2"}).json()['data']
                            print(response['id'])
                            to_add = s.post(f"{direct_us_url}/items/missing_alert",headers=headers,json={"frame":response['id'],"location":location_camera,"missing_detected":name,'cctv_location':"POINT (77.3133 23.1453)"}).json()
                            
                            print(name,time.ctime(),location_camera,"POINT (77.3133 23.1453)")
                        print(name,face_distances)
                except Exception as e:
                    print(e,'HERE')
